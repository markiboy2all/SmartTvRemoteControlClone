...
// Type: com.samsung.multiscreen.device.requests.GetChannelRequest
...
namespace com.samsung.multiscreen.device.requests
{
  public class GetChannelRequest
  {
    ...
    public GetChannelRequest(Uri endpoint, string channelId, DeviceAsyncResult<Channel> callback)
    {
    ...
    protected internal virtual void parseResponse(HttpWebResponse response)
    {
      try
      {
        ...
        Logger.Debug("GetChannelRequest result rpcMessage: " + (object) withJsonData);
        if (withJsonData.IsError())
        {
          Logger.Debug("GetChannelRequest result rpc error: " + (object) withJsonData.GetError());
          this.callback.OnError(DeviceError.CreateWithJSONRPCError(withJsonData.GetError()));
        }
        else
          this.callback.OnResult(new Channel(ChannelInfo.createWithMap(withJsonData.GetResult()), new ConnectionFactory()));
      }
      catch (Exception ex)
      {
        this.callback.OnError(new DeviceError(ex.ToString()));
